...
@section('title', __('pages.create_page'))
            ...
            <div class="col-12">
                <div class="card border-grey border-lighten-3 m-0">
                    <div class="card-header border-0 pb-0 d-flex flex-column flex-md-row justify-content-between align-items-center">
                        <h4 class="card-title mb-2 mb-md-0">{{ __('pages.create_page') }}</h4>
                            ...
                            <form action="{{ route('dashboard.pages.store') }}" method="POST">
                                ...
                                <div class="form-group">
                                    ...
                                    <input
                                        ...
                                        required
                                    >
                                    <small class="form-text text-muted">{{ __('pages.slug_help') }}</small>
                                    @error('slug')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="d-flex justify-content-end mt-3">
                                    <a href="{{ route('dashboard.pages.index') }}" class="btn btn-outline-secondary mr-1">
                                        {{ __('messages.cancel') }}
                                    </a>
                                    <button type="submit" class="btn btn-info">
                                        <i class="ft-save"></i> {{ __('pages.save_page') }}
                                    </button>
                                </div>
                            </form>
                        </div>
